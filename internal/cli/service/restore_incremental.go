...
func newRestoreIncrementalCmd(rc *runCtx) *cobra.Command {
	...
	cmd.AddCommand(
		newRestoreIncrementalStartCmd(rc),
	)
...
func newRestoreIncrementalStartCmd(rc *runCtx) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			jobID, err := handler.RestoreIncremental(cmd.Context(), &f.RestoreRequest)
			if err != nil {
				return err
			}
