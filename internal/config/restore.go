...
func logRestoreConfig(logger *slog.Logger, params *RestoreServiceConfig, restoreConfig *backup.ConfigRestore) {
	logger.Info("initialized restore config",
		...
		slog.Bool("noRecords", restoreConfig.NoRecords),
		slog.Bool("noIndexes", restoreConfig.NoIndexes),
		slog.Bool("noUdfs", restoreConfig.NoUDFs),
		slog.Bool("disableBatchWrites", restoreConfig.DisableBatchWrites),
		slog.Int("batchSize", restoreConfig.BatchSize),
		slog.Int("maxAsyncBatches", restoreConfig.MaxAsyncBatches),
		slog.Int64("extraTtl", restoreConfig.ExtraTTL),
		slog.Bool("ignoreRecordError", restoreConfig.IgnoreRecordError),
		slog.Bool("applyMetadataLast", restoreConfig.ApplyMetadataLast),
	)
}
