...
func TestBackup_ToModelBackup_EmptyLists(t *testing.T) {
	...
	assert.Empty(t, model.NodeList)
	assert.Empty(t, model.PreferRacks)
	assert.Empty(t, model.PartitionList)
	assert.Empty(t, model.RackList)
}
