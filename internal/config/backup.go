...
func logBackupConfig(logger *slog.Logger, params *BackupServiceConfig, backupConfig *backup.ConfigBackup) {
	logger.Info("initialized scan backup config",
		...
		slog.String("partitionList", params.Backup.PartitionList),
		slog.Any("nodeList", backupConfig.NodeList),
		slog.Any("setList", backupConfig.SetList),
		slog.Any("binList", backupConfig.BinList),
		slog.Any("rackList", backupConfig.RackList),
		slog.Any("parallelRead", backupConfig.ParallelRead),
		slog.Any("parallelWrite", backupConfig.ParallelWrite),
		slog.Bool("noRecords", backupConfig.NoRecords),
		slog.Bool("noIndexes", backupConfig.NoIndexes),
		slog.Bool("noUdfs", backupConfig.NoUDFs),
		slog.Int("recordsPerSecond", backupConfig.RecordsPerSecond),
		slog.Int64("bandwidth", backupConfig.Bandwidth),
		slog.Uint64("fileLimit", backupConfig.FileLimit),
		slog.Bool("compact", backupConfig.Compact),
		slog.Bool("noTtlOnly", backupConfig.NoTTLOnly),
		slog.String("stateFileDst", backupConfig.StateFile),
		slog.Bool("continue", backupConfig.Continue),
		slog.Int64("scanPageSize", backupConfig.PageSize),
		slog.String("outputFilePrefix", backupConfig.OutputFilePrefix),
	)
}
...
func logXdrBackupConfig(logger *slog.Logger, params *BackupServiceConfig, backupXDRConfig *backup.ConfigBackupXDR) {
	logger.Info("initialized xdr backup config",
		...
		getEncryptionLog(params.Encryption),
		getCompressionLog(params.Compression),
		slog.Any("parallelWrite", backupXDRConfig.ParallelWrite),
		slog.Uint64("fileLimit", backupXDRConfig.FileLimit),
		slog.String("dc", backupXDRConfig.DC),
		slog.String("localAddress", backupXDRConfig.LocalAddress),
		slog.Int("localPort", backupXDRConfig.LocalPort),
		slog.String("rewind", backupXDRConfig.Rewind),
		slog.Int("maxThroughput", backupXDRConfig.MaxThroughput),
		slog.Duration("readTimeout", backupXDRConfig.ReadTimeout),
		slog.Duration("writeTimeout", backupXDRConfig.WriteTimeout),
		slog.Int("resultsQueueSize", backupXDRConfig.ResultQueueSize),
		slog.Int("ackQueueSize", backupXDRConfig.AckQueueSize),
		slog.Int("maxConnections", backupXDRConfig.MaxConnections),
	)
}
