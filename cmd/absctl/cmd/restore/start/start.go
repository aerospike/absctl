...
const (
	welcomeMessage = "Start a server-integrated restore on the Aerospike cluster."
	shortMessage   = "Start a server-integrated restore"
)
...
func NewCmd(shared *subcmd.SharedFlags, flagsRoot *flags.Root, appVersion, commitHash, buildTime string,
	...
	cmd.SetHelpFunc(func(_ *cobra.Command, _ []string) {
		newHelpFunction(flags.SectionTextUsageRestoreStart, ssbFlagSet, awsFlagSet)
	})
...
func newHelpFunction(usageText string, ssbFlagSet, awsFlagSet *pflag.FlagSet) {
	...
	fmt.Println("\nUse \"absctl restore --help\" for the full list of inherited flags.")
}
